...
- platform Perfetto trace metadata
...
アプリケーションレイヤーの開発だけで完結する領域ではない。SDV は platform / middleware / vehicle service / network protocol / update / telemetry を含むため、対象環境・SoC/BSP 側の責務分界を明確にする必要がある。

個人検証側 HMI 開発への直接影響は、短期的には以下になる。
...
- 対象環境実装の VHAL が該当 property を提供しているか。
- エミュレータ、reference device、本格適用 BSP で property availability が一致するか。
...
AAOS 25Q4 では、car data subscription events への logging が追加され、platform user connectivity logging が有効化されると説明されている。
...
この領域はアプリレイヤーではなく、BSP、kernel、vendor code、SELinux、storage encryption、lockscreen / credential UI に関わる。個人検証側 HMI としては、直接実装するよりも対象環境への確認項目に入れるべきである。
...
PoC 移植や対象環境折り込みでは、従来の「ビルド中に生成物を source tree に書く」タイプの処理が問題になる可能性がある。
...
## 個人検証側開発・対象環境折り込みへの影響分類

| 領域 | 個人検証側で主導しやすいこと | 対象環境連携が必要なこと |
| --- | --- | --- |
...
| Privacy / logging | 取得したい HMI event、panel event、task event の仕様化 | platform logging policy、permission、consent、Google ToS gating、監査ログ |
...
一方で、SDV、kernel/security、VHAL、SELinux、CTS/VTS/STS/ATS はアプリレイヤーだけでは完結しない。Android 17 への本格移行では、個人検証側が HMI/RRO/XML/アプリ/検証仕様を主導し、対象環境が platform integration / BSP / compliance を担う分担で進めるのが現実的である。
