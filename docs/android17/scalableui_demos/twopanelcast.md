# TwoPanelRROCast ScalableUI Demo Analysis

## 位置づけ

map_panel を常時表示し、app_panel を task open で前面表示する 2 panel 構成。

- Source: `packages/apps/Car/References/scalable-ui/codelab/TwoPanelRROCast`
- 種別: `codelab`
- Build module: `TwoPanelRROCast`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `app_panel` | `TaskPanel` | `@id/closed` | `@string/default_config` | `-` | `@+id/opened`, `@+id/closed` | - | `packages/apps/Car/References/scalable-ui/codelab/TwoPanelRROCast/res/xml/app_panel.xml` |
| `map_panel` | `TaskPanel` | `@id/opened` | `@array/nav_components` | `-` | `@+id/map_base`, `@+id/opened`, `@+id/closed` | - | `packages/apps/Car/References/scalable-ui/codelab/TwoPanelRROCast/res/xml/map_panel.xml` |
...
この demo では XML 上で 3 個の Transition が確認できる。主なものは以下。
...
2. `m TwoPanelRROCast` で RRO module を build する。複数 module がある場合は `TwoPanelRROCast` を確認する。
...
- `packages/apps/Car/References/scalable-ui/codelab/TwoPanelRROCast/res/xml/map_panel.xml`
