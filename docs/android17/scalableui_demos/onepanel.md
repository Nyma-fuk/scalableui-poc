# OnePanelRRO ScalableUI Demo Analysis

## 位置づけ

1つの app_panel だけで TaskPanel の open / close を確認する最小構成。

- Source: `packages/apps/Car/References/scalable-ui/codelab/OnePanelRRO`
- 種別: `codelab`
- Build module: `OnePanelRRO`
...
TaskPanel は 1 個、DecorPanel は 0 個、SystemWindow は 0 個確認できる。

## Panel 一覧

| Panel | 種類 | defaultVariant | role | controller | variants | keyframes | source |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `app_panel` | `TaskPanel` | `@id/closed` | `@string/default_config` | `-` | `@+id/opened`, `@+id/closed` | - | `packages/apps/Car/References/scalable-ui/codelab/OnePanelRRO/res/xml/app_panel.xml` |
...
|                                                  |
|  app_panel                                       |
|  closed -> opened by _System_TaskOpenEvent       |
|  opened -> closed by _System_OnHomeEvent         |
+--------------------------------------------------+
```
...
この demo では XML 上で 2 個の Transition が確認できる。主なものは以下。
...
2. `m OnePanelRRO` で RRO module を build する。複数 module がある場合は `OnePanelRRO` を確認する。
...
- `packages/apps/Car/References/scalable-ui/codelab/OnePanelRRO/res/xml/app_panel.xml`
