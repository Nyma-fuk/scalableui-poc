...
- Android17 では ScalableUI 本体は存在するが、この PoC の `declarative-multipanel` PoC product/RRO が AOSP 標準に入ったわけではない。
...
`StateManager.addState()` は存在するため、runtime 追加の下回りとして使える可能性はある。ただし Android17 の標準 config flow は XML/DCF reload であり、任意 panel の UI、geometry、永続化、app picker、policy まで含む runtime panel generation は PoC/custom と分類する。
...
- `StateManager.addState()` があるため runtime panel generation が本格適用標準機能として完成している、と説明する。
