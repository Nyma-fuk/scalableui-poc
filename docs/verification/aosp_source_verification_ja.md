...
| TaskPanel | AAOS15 LTS3 では `BasePanel` 系、Android17 では `SysUIPanel` 系。いずれも root task stack / task を扱い、Activity 直接ではない。 | Correct | `packages/apps/Car/SystemUI/src/com/android/systemui/car/wm/scalableui/panel/TaskPanel.java`, `docs/verification/aaos17_scalableui_source_verification_ja.md` |
