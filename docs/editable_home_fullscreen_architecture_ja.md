...
- 自社 app / 特定 app のみ: `duplicate semantics` を個別対応
...
検証注記: AOSP の `WindowContainerTransaction.reparent(...)` / `reparentTasks(...)` は存在します。ただし現在の live ScalableUI source だけでは、既存 task を Panel A から `app_panel` へ直接 reparent する標準実装は未確認です。この節は PoC custom routing / OEM policy として扱います。
