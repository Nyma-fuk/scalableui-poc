...
本資料は、Android 17 への移植作業を開始する前の一次整理である。Android 17 の full checkout / build / emulator 起動はまだ実施していない。
...
2. `BasePanel` が `SysUIPanel` へ rename され、Panel 抽象の命名が変わっている。
...
この PoC の PoC は Android 16 QPR2 上で、RRO と一部 SystemUI patch によって multipanel HMI を成立させている。一方、Android 17 では ScalableUI 本体側に大きな整理が入っているため、Android 16 向けに足した patch の一部は、そのまま再適用するのではなく、Android 17 の新しい設計に載せ替える必要がある。
...
| Task / WMShell | アプリを載せる task の作り方が変わった | TaskPanel / TaskView / ShellTaskOrganizer の移植が必要 |
| Product / RRO | この PoC の PoC product は Android 17 AOSP には入っていない | RRO / StubCarLauncher / product 定義は再適用する |
...
### 1. ScalableUI README が追加された

| 項目 | 内容 |
| --- | --- |
| 何が変わったか | Android 17 では ScalableUI 配下に README が追加され、ScalableUI の役割、構成要素、処理の流れが公式に説明されるようになった |
| なぜ重要か | これまでコードを読まないと分かりづらかった「どこが責務を持つのか」が明文化された |
| PoCへの影響 | 今後の設計説明や対象環境依頼では、この README の責務分解をベースに説明できる |
| 説明用の一言 | Android 17 では、ScalableUI の考え方がドキュメント化され、移植判断の基準にしやすくなった |
...
| PoCへの影響 | All Apps overlay、最大化、Home 復帰を「WM変更が必要なもの」と「Surface animationで済むもの」に分ける必要がある |
| 説明用の一言 | Android 17では、画面構成を変える処理と、見た目だけを動かす処理を分けて考える必要がある |
...
この整理により、この PoC の PoC の以下の要件は再分類が必要になる。
...
### 3. `BasePanel` が `SysUIPanel` に変わった

| 項目 | 内容 |
| --- | --- |
| 何が変わったか | Android 16 QPR2 では `BasePanel` だった抽象が、Android 17 では `SysUIPanel` に rename された |
| なぜ重要か | Panel の基本クラス名が変わるため、SystemUI patch がそのまま当たらない可能性が高い |
| PoCへの影響 | `BasePanel` を参照する PoC patch、controller、factory の修正が必要 |
| 説明用の一言 | Panel の土台クラスの名前と整理が変わったため、移植時に機械的な修正が必ず発生する |

これは機能追加というより、ScalableUI 内部の名前と構造の整理である。ただし、移植時にはビルドエラーとして出やすい。Android 17 対応の最初の作業で潰すべき差分である。
...
| PoCへの影響 | runtime panel generation や controller 初期化順序の前提を見直す必要がある |
| 説明用の一言 | Panel定義を「追加していく」より、「設定として読み直す」設計に寄っている |
...
### 5. `PanelConfigReadStateMonitor` が追加された

| 項目 | 内容 |
| --- | --- |
| 何が変わったか | Panel config の読み込みが完了したかを監視する monitor が追加された |
| なぜ重要か | config が読み終わる前に controller や event が動くと、対象 Panel が見つからない可能性がある |
| PoCへの影響 | `SHOW_ALL_APPS` や `MAXIMIZE_PANEL` などの event を、config ready 後に処理する必要がある |
| 説明用の一言 | Android 17では、Panel定義が準備できてから動作を開始する安全装置が追加された |

この PoC の PoC では、起動直後に固定 Panel、All Apps、app picker などが関係する。Android 17 では、初期化順序がより重要になるため、boot直後の smoke test で config ready と Panel 表示を確認する必要がある。
...
| なぜ重要か | All Apps、最大化、Home 復帰など、この PoC の HMI 操作の中心にある |
| 追加された考え方 | direct animation、historical event、focus calculation、current state の強制同期 |
| PoCへの影響 | Android 16 向けに入れた direct animation patch は、Android 17 の標準機能で置き換えられる可能性がある |
| 説明用の一言 | Panelの動かし方の中核が強化されているので、既存patchを足すのではなくAndroid 17流に載せ替えるべき |
...
| PoCへの影響 | Android 16向けの target panel routing patch が conflict する可能性が高い |
| 説明用の一言 | アプリ起動をどのPanelに流すかを判断する関所が変わっている |

この PoC の PoC では、「All Appsから起動した通常アプリは `app_panel` に出す」「Settingsは固定settings panelではなくfullscreen寄りpanelへ出す」といったルールを入れている。この判断は delegate と task routing に関わるため、Android 17 の変更に合わせて再実装が必要になる。
...
| なぜ重要か | 個人検証側アプリや AppGrid から送る HMI event の入口だから |
| PoCへの影響 | `SHOW_ALL_APPS`、`DISMISS_ALL_APPS`、`MAXIMIZE_PANEL`、`RESTORE_HOME` の mapping を再確認する |
| 説明用の一言 | ボタン操作やアプリ操作をScalableUIに伝えるイベント経路が変わっている |
...
| PoCへの影響 | 既存アプリ再選択で最大化、既存task再利用、target panel routing の実装を見直す |
| 説明用の一言 | Panelにどのアプリtaskが載っているかを管理する部分が変わっている |

ここは、ユーザー体験に直結する。たとえば、地図がすでに小さいPanelに出ている状態で地図を再度起動した場合、この PoC では「同じアプリを増やす」のではなく「既存Panelを最大化する」挙動を目指している。この挙動は `TaskPanel` / `TaskPanelInfoRepository` / routing policy に関係する。

### 10. `SetupPanelController` が追加された

| 項目 | 内容 |
| --- | --- |
| 何が変わったか | setup 用の Panel controller が追加された |
| なぜ重要か | ScalableUI controller が用途別に増えており、controller module の構成が変わっている |
| PoCへの影響 | 個人検証側 controller を追加する場合、Android 17 の module / Dagger 構成に合わせる必要がある |
| 説明用の一言 | Android 17では、Panel制御を用途別controllerとして整理する方向が進んでいる |

この PoC で将来 `AllAppsPanelController`、`MaximizePanelController`、`AppAssignmentController` のような個人検証側controllerを作る場合、Android 17 の controller module 構成を参考にするのがよい。

### 11. GripBar が方向別に分かれた

| 項目 | 内容 |
| --- | --- |
| 何が変わったか | `GripBar.java` が `GripBarBase.java` になり、`HorizontalGripBar.kt` と `VerticalGripBar.kt` が追加された |
| なぜ重要か | Panel resize / drag UI の表現が方向別に整理された |
| PoCへの影響 | 将来 grip resize やPanel移動をやる場合、Android 17のクラス構成に合わせる |
| 説明用の一言 | Panelを掴んで動かすUI部品が、横向き・縦向きで整理された |
...
| PoCへの影響 | All Appsを常に最前面に出す設計では、z-order / input / focus の確認が必要 |
| 説明用の一言 | アプリPanelだけでなく、SystemUIのバーや通知との重なり方も見直しが必要 |
...
| PoCへの影響 | `CarSystemUI` module build、RRO build、StubCarLauncher build のCIをAndroid 17向けに更新する |
| 説明用の一言 | コードだけでなく、ビルドの組み立て方も変わっている |
...
| PoCへの影響 | TaskPanel / TaskView / task routing を触るpatchは再設計が必要 |
| 説明用の一言 | アプリを載せるためのtaskコンテナの作り方が変わっている |
...
### 15. `TaskViewRootTask.kt` が追加された

| 項目 | 内容 |
| --- | --- |
| 何が変わったか | TaskView が管理する root task 情報を表す interface が追加された |
| なぜ重要か | TaskView / RemoteTaskView / TaskPanel の境界を整理する材料になる |
| PoCへの影響 | 既存ドキュメントの「Panel -> TaskPanel -> Task -> Activity」モデルをAndroid 17でも再確認する |
| 説明用の一言 | TaskViewがどのroot taskを管理しているかを扱うための抽象が追加された |
...
### 16. Android 17 AOSPにはこの PoC のPoC product/RROは存在しない

| 項目 | 内容 |
| --- | --- |
| 何が変わったか | Android 17 AOSPには `car_product/scalableui_declarative_multipanel` は存在しない |
| なぜ重要か | この PoC のHMIはAOSP標準機能として入ったわけではない |
| PoCへの影響 | product mk、RRO、StubCarLauncher、AppGrid、app picker は新規差分として再適用する |
| 説明用の一言 | Android 17に上げてもPoC HMIが自動で入るわけではなく、この PoC の差分は再度持ち込む必要がある |

この点は説明で誤解されやすい。Android 17でScalableUI本体は更新されているが、この PoC で作った `declarative-multipanel` のproductやRROがAOSPに入ったわけではない。したがって、Android 17対応では「AOSP側のScalableUI更新に追従する作業」と「この PoC のPoC差分を再適用する作業」の両方が必要である。
...
| `BasePanel.kt -> SysUIPanel.kt` | rename | PoC patch で `BasePanel` を参照している場合は修正必須 |
...
| `GripBar.java -> GripBarBase.java` | rename | grip UI / controller を触る場合は修正必須 |
...
この README は、この PoC で作成した「controller / task event / telemetry」整理と方向性は合っている。一方で、Android 17 では Window State と Surface の分離がより明確に書かれているため、移植時には以下の設計判断を明示する必要がある。
...
| `BasePanel.Factory` | `SysUIPanel.Factory` |
...
1. この PoC の `MAXIMIZE_PANEL` / `RESTORE_HOME` / `SHOW_ALL_APPS` event は、Android 17 の `PanelTransitionCoordinator` に合わせて「WM change 必要」か「direct animation で足りる」かを分類する。
...
| 確認項目 | 理由 |
| --- | --- |
| event 受信元 | 個人検証側 app / StubCarLauncher / AppGrid から送る event を受けられるか |
...
3. `TaskViewRootTask` が追加されたため、TaskView が管理する root task 情報取得の公式経路が増えている。RemoteTaskView / TaskView / TaskPanel の境界を再確認する。
4. Back / Home 復帰の挙動は `onBackOnTaskRoot(...)` の詳細引数を考慮する。

## packages/services/Car / product / RRO 差分

Android 17 AOSP の `packages/services/Car` には、この PoC の PoC product `car_product/scalableui_declarative_multipanel` は存在しない。
...
| `sdk_car_scalableui_declarative_multipanel_x86_64` product | `device/generic/car` Android 17 側 product 構成に合わせて再作成 |
...
| 優先 | 確認対象 | 理由 |
| --- | --- | --- |
| 1 | `BasePanel` -> `SysUIPanel` rename | patch conflict 確実。参照修正が必要 |
...
7. `BasePanel` 参照を `SysUIPanel` に更新する。
...
| 項目 | 状態 |
| --- | --- |
| Android 17 full checkout | 未実施 |
| Android 17 への PoC patch apply | 未実施 |
| Android 17 module build | 未実施 |
| Android 17 emulator image build | 未実施 |
| Runtime validation | 未実施 |
| RRO resource 名の完全照合 | 未実施 |
| SystemUI event 実装の詳細追跡 | 一次差分のみ |
