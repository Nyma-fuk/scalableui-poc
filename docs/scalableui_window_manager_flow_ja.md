...
> Supplier-facing note: この文書では、個人PC上の絶対パスや一時ファイルパスを使わず、AOSP source relative path または `<AAOS17_ROOT>` / `<SCALABLEUI_POC_ROOT>` のような placeholder で表記します。
...
評価 artifact は repository に含めず、環境ごとの証跡ディレクトリに保存します。サプライヤー提示時は個人環境の絶対パスではなく、次のような placeholder で記載します。
