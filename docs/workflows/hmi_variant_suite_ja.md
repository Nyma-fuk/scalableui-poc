...
- 挙動修正のたびに emulator で起動評価し、build 成功だけで完了扱いにしない
...
- runtime 挙動に影響する変更を入れたら、対象 variant を emulator で起動して評価する
- 少なくとも 1 枚の screenshot と、必要なら `dumpsys activity` / `visible window` を残す
- docs には「build 成功」だけでなく「emulator で何を確認して何が通ったか」を追記する
...
`PanelMenuActivity` は `widget-workspace` の menu panel からユーザーが選んだ panel に表示する app を起動し、PoC custom routing で panel 内の app 入れ替えを確認します。token reparent routing は historical patch の検証項目であり、AOSP 標準 ScalableUI 機能として扱いません。
`editable-home` は現在、`HomeEditActivity` 中心の preset 切り替えではなく、3 panel に対して常時見える header と grip を持つ multipanel home として再設計されています。header から fullscreen app picker を開き、任意のインストール済み app を panel に割り当て、grip の drag で左右 split / 右 column 内の上下 split を連続変更し、その結果を `SharedPreferences` に保存します。
...
実機向け UI ではなく、panel bounds / routing / layout / widget 操作を試すためのものです。
...
- demo app は placeholder / sample app であり、本格適用 UI 品質の app ではありません
...
```powershell
<WINDOWS_ANDROID_SDK>\emulator\emulator.exe -list-avds
```
...
Start-Process -FilePath '<WINDOWS_ANDROID_SDK>\emulator\emulator.exe' `
  -ArgumentList '-avd','<AVD_NAME>','-no-snapshot-load'
<WINDOWS_ANDROID_SDK>\platform-tools\adb.exe devices -l
```

同じ AVD が既に起動中なら、先に emulator console から stop してから再起動します。

```powershell
<WINDOWS_ANDROID_SDK>\platform-tools\adb.exe -s <DEVICE_SERIAL> emu kill
Start-Process -FilePath '<WINDOWS_ANDROID_SDK>\emulator\emulator.exe' `
  -ArgumentList '-avd','<AVD_NAME>','-no-snapshot-load'
```
