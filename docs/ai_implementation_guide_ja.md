...
主対象は `declarative-multipanel`。

Android17では、PoC専用productを増やさず、標準 `sdk_car_x86_64-trunk_staging-userdebug` にPoC差分を追加する。

`dynamic-workspace` は過去の大きな実験であり、runtime panel generation / assignment / persistence の知見として参照する。

### declarative-multipanel の目的

- RRO/XMLで固定panel、variant、transitionを宣言する
- Framework / CarService / CarSystemUI RROでScalableUIを有効化する
- 標準CarLauncherをPoC用のStub HOME hostで分離する
- AppGridをpanelとして扱い、選択アプリを指定panelへroutingする
- All Apps、最大化、Home復帰などを段階的に追加する

### dynamic-workspace の過去目的
...
現行 `declarative-multipanel` では、固定的な panel / variant は RRO XML で定義する。
動的 panel 群を SystemUI runtime code で追加する実装は `dynamic-workspace` の historical custom 実装として扱う。
...
WorkspaceRuntimeLayoutController
  ...
  |     StateManager.addState, Variant runtime bounds, BasePanel.update (AAOS15/old patch)
  |
  +-- WorkspaceTaskRouter
...
- `WorkspacePanelStateController` は `StateManager` / `PanelPool` / AAOS15系の `BasePanel.update(...)` に依存しており、Android17移植時は `SysUIPanel` / `TaskPanel` / `PanelTransitionCoordinator` 前提で再確認する。
