...
ScalableUI は通常アプリ向け SDK ではなく、CarSystemUI / WMShell / root task stack / Task 管理と結びつく AAOS システム側の UI 制御機構である。そのため、実機環境で ScalableUI を利用するには、対象環境側で SystemUI、RRO、priv-app、署名、権限、CarLauncher、WindowManager 周辺を統合する必要がある。
