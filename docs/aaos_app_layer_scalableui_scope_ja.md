...
ScalableUI は通常アプリ向け SDK ではなく、CarSystemUI / WMShell / TaskView / Task 管理と結びつく AAOS システム側の UI 制御機構である。そのため、実機環境で ScalableUI を利用するには、対象環境側で SystemUI、RRO、priv-app、署名、権限、CarLauncher、WindowManager 周辺を統合する必要がある。

一方で、ScalableUI 上に表示される各アプリの画面設計、パネルサイズ変化に追従する UI、パネル内での操作体験、アプリ選択や状態管理に必要なアプリ側のデータモデルは、個人検証側でかなり具体的に作り込める。

さらに、個人検証側で Panel XML、RRO、SystemUI 側の controller、transition handler、event bridge などを作成し、対象環境へ「AAOS プラットフォーム差分」として提供することも可能である。ただし、この場合の成果物は通常アプリ APK ではなく、AOSP/AAOS ソースツリーへ取り込む patch、RRO module、priv-app、product makefile 差分として扱う必要がある。

```mermaid
flowchart TD
    A[個人検証側開発アプリ APK] --> B[Activity / Service / Intent]
    B --> C[対象環境統合済み Launcher / SystemUI]
    C --> D[ScalableUI Panel 定義]
    D --> E[TaskPanel / TaskView 相当の埋め込み領域]
    E --> F[Task]
    F --> G[Activity 表示]

    H[個人検証側 HMI 仕様] --> I[Panel Catalog / Event Catalog / App Assignment]
    I --> C
    I --> D
```

## 個人検証側で作成して対象環境へ提供できるプラットフォーム成果物

アプリケーションレイヤーだけでは ScalableUI の Panel 制御は完結しないが、個人検証側に AOSP/AAOS ソースを編集・ビルドできる環境がある場合、ScalableUI 向けのプラットフォーム差分を自作して対象環境に渡すことはできる。

この場合、責務分担は「個人検証側が設計と参照実装を作る」「対象環境が本格適用 AAOS ベースへ移植・検証・署名・権限統合する」という形になる。

| 提供物 | 個人検証側作成可否 | できるようになること | 対象環境側で必要な作業 |
| --- | --- | --- | --- |
| Panel 定義 XML | 可能 | Home 上の Panel 数、位置、サイズ、初期表示、表示 variant を個人検証側 HMI 意図どおりに定義できる | 対象 AAOS ブランチの ScalableUI resource へ統合 |
...
| default activity assignment | 可能 | 地図、メディア、カレンダーなどをどの Panel に初期表示するかを個人検証側アプリ package/class 単位で指定できる | 実車 package/class 名、署名、起動権限との整合確認 |
...
| task event 名・extra 定義 | 可能 | 個人検証側アプリや AppGrid から `SHOW_ALL_APPS`、`MAXIMIZE_PANEL` のような HMI イベントを送る契約を作れる | SystemUI event bridge、権限、送信元制限の実装 |
...
| build patch set | 可能 | 対象環境が同じ HMI 差分を再現できる単位で提供でき、レビューや rebase の起点にできる | 対象環境の AAOS baseline へ cherry-pick / rebase |
...
| 形式 | 内容 | できるようになること | 用途 |
| --- | --- | --- | --- |
| AOSP patch set | `packages/apps/Car`、`device/generic/car`、`frameworks/base` などへの差分 | 対象環境が個人検証側 HMI 実装をソース差分として取り込める | 対象環境統合の起点 |
...
| integration guide | 適用手順、依存ブランチ、権限、ビルド対象 | 対象環境側の作業抜け、branch 取り違え、overlay 未適用を減らせる | 対象環境作業指示 |
| smoke test | 起動確認、Panel 表示、event 動作確認 | 個人検証側と対象環境で同じ合否基準を使える | 受け入れ基準 |

```mermaid
flowchart LR
    A[個人検証側 HMI 設計] --> B[Panel XML / RRO]
...
この提供方式では、個人検証側の成果物は「そのまま APK として入れれば動くもの」ではない。ScalableUI は SystemUI と WindowManager/Shell の上で動くため、対象環境側で次の確認が必要になる。
...
したがって、個人検証側で XML/RRO/controller/transition を作ることは可能だが、本格適用上は「対象環境統合を前提にした参照実装」として扱うのが安全である。

## RRO / Panel XML を内製開発するための進め方

現時点で RRO や Panel XML の実機環境への取り込みは対象環境主担当になりやすいが、開発責任をすべて対象環境へ寄せる必要はない。個人検証側で ScalableUI 用の RRO、Panel XML、transition、event contract、controller の参照実装を作り、対象環境には本格適用 AAOS への取り込み、署名、権限、SELinux、実機検証を担当してもらう体制へ段階的に移行できる。

目標は「対象環境に HMI を考えて実装してもらう」状態から、「個人検証側が HMI の実装単位まで定義し、対象環境は車両・実機環境へ統合する」状態へ移すことである。

```mermaid
flowchart TD
    A[個人検証側 HMI 要求] --> B[Panel Catalog]
    B --> C[RRO / Panel XML]
    C --> D[AAOS emulator build]
    D --> E[smoke test / screenshot evidence]
    E --> F[patch set / integration guide]
    F --> G[対象環境本格適用 AAOS へ統合]
    G --> H[実機評価 / 車両要件調整]
...
| レベル | 個人検証側が持つ範囲 | 対象環境が持つ範囲 | 到達状態 |
| --- | --- | --- | --- |
| Level 0 | HMI 仕様、画面遷移、アプリ UI | RRO/XML/controller 実装 | 従来型の仕様依頼 |
| Level 1 | Panel catalog、package/class assignment、bounds 設計 | XML/RRO 実装、統合 | 個人検証側が ScalableUI 構成をレビュー可能 |
| Level 2 | RRO module、Panel XML、default activity XML | product 組み込み、署名、overlay enable | 個人検証側が表示構成を実装可能 |
| Level 3 | transition/state/event 定義、Stub HOME/AppGrid | SystemUI 接続、priv-app 権限、SELinux | 個人検証側が HMI 動作を参照実装可能 |
| Level 4 | controller / event bridge / task routing patch | 対象 branch 追従、車両統合、実機検証 | 個人検証側が ScalableUI HMI の実装主担当 |

推奨開始点は Level 2 である。RRO と Panel XML は比較的境界が明確で、HMI 意図をコード化しやすい。Level 3 以降は SystemUI と task 制御に入るため、対象環境との API / patch contract を先に決める必要がある。

### 個人検証側に必要な開発環境

RRO / XML を内製するには、通常の Android アプリ開発環境だけでは足りない。最低限、AOSP/AAOS ソースツリーと emulator build 環境を個人検証側に持つ必要がある。

| 必要なもの | 目的 |
| --- | --- |
| 対象 AAOS baseline | 対象環境と同じ branch / tag / manifest に合わせる |
| AOSP build 環境 | RRO、SystemUI、CarLauncher、product image をビルドする |
| emulator 起動環境 | Panel 表示、overlay 適用、task routing を個人検証側で確認する |
| ScalableUI source knowledge | Panel XML、variant、transition、StateManager の理解 |
| patch export 手順 | 対象環境へ渡せる差分に整える |
| smoke test | 受け入れ条件を自動化する |

### 個人検証側リポジトリで管理すべき成果物

対象環境へ渡す前に、個人検証側リポジトリ上では以下を一体で管理する。

| ディレクトリ / 成果物 | 役割 | できるようになること |
| --- | --- | --- |
| `docs/` | HMI 設計、責務分担、integration guide | 対象環境と同じ前提で仕様・制約・責務をレビューできる |
| `variants/<name>/docs/` | Variant ごとの HMI 仕様、評価結果 | 仕向けやデザイン案ごとの HMI 差分を比較できる |
| `variants/<name>/patches/` | AOSP/AAOS へ適用する patch set | 対象環境へソース差分としてそのまま渡せる |
| `variants/<name>/rro/` または patch 内 RRO | Panel XML、overlay config、dimens、strings | RRO/XML を個人検証側で編集・レビューし、表示構成を再現できる |
...
### 対象環境へ渡す integration contract

個人検証側が RRO/XML を主導する場合、対象環境への依頼は曖昧な HMI 仕様ではなく、取り込み単位に分解する。
...
RRO / Panel XML / controller を内製する場合、個人検証側のゴールは「HMI 仕様を作る」だけではなく、「AAOS emulator 上で再現できる patch set と検証証跡を作る」ことに置く。対象環境への依頼は、その成果物を対象 AAOS baseline へ取り込み、車両依存要素を調整し、実機検証する作業として定義する。
...
flowchart TD
    ...
    C --> F[個人検証側 AAOS source tree]
    D --> F
    E --> F
    G[個人検証側 app APK / priv-app] --> F
    ...
    L --> M[Integration intake]
    M --> N[Integration AAOS baseline rebase]
...
個人検証側内で持つべき検証環境は、アプリ単体、AAOS emulator、対象環境統合、実機の 4 層に分ける。

| 環境 | 個人検証側で確認すること | 対象環境で確認すること | 合格条件 |
| --- | --- | --- | --- |
| App local / Android Studio | 個人検証側アプリの画面、レスポンシブ UI、基本操作 | 不要 | Panel サイズ相当の幅・高さで UI が破綻しない |
| 個人検証側 AAOS emulator | RRO 適用、Panel 表示、default activity、All Apps、最大化、Home 復帰 | 任意で再実行 | HMI smoke test が pass し、証跡が残る |
| 対象環境 AAOS integration build | patch 適用、product package、privapp permission、overlay enable | 主担当 | 個人検証側 emulator と同等の HMI smoke test が pass する |
| Bench / 実車 | focus、input、multi-display、user switching、走行中制限、性能 | 主担当 | 車両要件と安全要件を満たす |

### 個人検証側 CI の推奨フロー

個人検証側 CI では、RRO/XML の syntax だけでなく、AAOS image 上で実際に overlay と Panel が効いていることまで確認する。
...
| Artifacts upload | image、patch、log、screenshot を保存 | 対象環境への再現材料不足 |

### 対象環境折り込み依頼フロー

対象環境へ渡すときは、HMI 仕様書だけでなく、CI が pass した patch set、適用手順、受け入れテスト、既知の未対応事項をセットで渡す。

```mermaid
sequenceDiagram
    participant InHouse as 個人検証側 HMI/Platform team
    participant CI as 個人検証側 CI
    participant Integration as 対象環境
    participant Vehicle as Bench/Vehicle

    InHouse->>CI: RRO/XML/controller patch を push
    CI-->>InHouse: build, emulator smoke, evidence
    InHouse->>Integration: patch set, integration guide, smoke test, evidence を提供
    Integration->>Integration: 対象 AAOS baseline へ rebase
    Integration->>Integration: signing, privapp, SELinux, product makefile を調整
    Integration->>CI: 可能なら同じ smoke test を実行
    Integration->>Vehicle: bench / vehicle image を投入
    Vehicle-->>Integration: focus, input, UX restriction, performance 結果
    Integration-->>InHouse: 差分、失敗ログ、車両制約を返却
...
| ブランチ | 用途 | merge 条件 |
| --- | --- | --- |
| `main` | 対象環境へ渡せる安定版 | docs、patch、smoke evidence が揃っている |
...
| `integration/<baseline>` | 対象環境 baseline への取り込み確認 | 対象環境側 conflict と調整点が記録済み |

### 対象環境へ渡す依頼パッケージ

| 成果物 | 対象環境ができること |
| --- | --- |
| patch set | 本格適用 AAOS tree へ差分を適用できる |
| integration guide | 追加 module、product package、overlay target、権限を確認できる |
| smoke test | 個人検証側と同じ観点で最低限の合否判定ができる |
| screenshot / log evidence | 個人検証側環境での正しい表示状態と比較できる |
...
ScalableUI で扱うべき対象は、アプリの中身ではなく、アプリを表示する Panel、Task、状態遷移、表示レイヤー、起動経路である。個人検証側で手を加えられる範囲は、RRO/XML、event contract、controller、task routing patch のどこまで持つかで変わる。
...
| 領域 | ScalableUI としてできること | 個人検証側で手を加えられる範囲 | 主な制約 |
| --- | --- | --- | --- |
...
| Default activity | 起動時にどの Activity をどの Panel へ載せるかを指定する | package/class を個人検証側アプリに合わせて定義できる | 実機 package 名、署名、起動権限との整合が必要 |
...
| Task event | All Apps、最大化、復帰、アプリ割当などの HMI 意図をイベント化する | event 名、extra、送信元、受信先を個人検証側で設計できる | event bridge と送信元制限が必要 |
...
| Task routing | 起動済み Task の再利用、指定 Panel への表示、最大化時の扱いを決める | policy と一部実装を個人検証側で作れる | ActivityTaskManager / WMShell 権限が必要 |
...
| Telemetry | Panel event、app launch request、transition 成功/失敗を記録する | event schema と記録ポイントを個人検証側で定義できる | privacy、consent、security 設計が必要 |

### Controller で具体的に実装できること

Controller は、XML だけでは表現しにくい「判断」と「記憶」と「複数コンポーネントの連動」を担う。ScalableUI を HMI として使い込む場合、controller は個人検証側 HMI ロジックの中心になり得る。
...
| Controller + task routing | ShellTaskOrganizer、TaskView/TaskPanel 周辺、launch policy | Task 再利用、reparent、focus、最大化 | 高い |
| Framework / WMShell 改修 | frameworks/base、WMShell | 既存 API で足りない挙動の追加 | 非常に高い |

実機向けには、まず `RRO only` と `RRO + event contract` を個人検証側主担当にし、`RRO + controller` を参照実装として持つのが現実的である。`Controller + task routing` 以降は車両固有の focus、multi-display、process death、UX restriction との結合が強いため、対象環境と共同開発にする。
...
ただし、ScalableUI は analytics 基盤ではない。ScalableUI が直接提供するのは Panel 定義、Task 配置、状態遷移、表示制御であり、データ収集を行うには SystemUI 側 controller、Launcher/AppGrid、個人検証側アプリ、必要に応じて CarService 側へ明示的に計測ポイントを入れる必要がある。
...
| データ | 実現性 | 収集ポイント | 備考 |
| --- | --- | --- | --- |
| All Apps を開いた | 高い | AppGrid button / SystemUI event bridge | 個人検証側実装の event として取りやすい |
...
| 個人検証側アプリ内の操作 | 高い | 個人検証側アプリ内 analytics / event log | 最も正確に取れる |
...
| 他アプリの UI 内容 | 非推奨 | Accessibility などに頼る設計は実機 HMI analytics として避けるべき |

### 実装方式

データ収集を行う場合は、ScalableUI の event / transition 設計と同時に telemetry event schema を定義する。

| 実装箇所 | 取れるデータ | 個人検証側実装可否 | 対象環境統合 |
| --- | --- | --- | --- |
...
| 個人検証側アプリ | 画面遷移、ボタン操作、設定変更 | 可能 | 通常 APK として実装可能 |
...
利用状況データは HMI 改善に有用だが、本格適用では privacy、consent、retention、security を設計に含める必要がある。
...
| 送信制御 | offline、工場診断、開発 build、本格適用 build を分ける |
| セキュリティ | ログ改ざん、過剰権限、第三者アプリからの event spoofing を防ぐ |
| 法規・地域差 | 市場ごとの privacy / data protection 要件を確認する |

実務的には、ScalableUI controller から取るのは「Panel と Task の状態」、Launcher/AppGrid から取るのは「アプリ起動要求」、個人検証側アプリから取るのは「アプリ内操作」に分けるのがよい。他社アプリ内部の細かい操作を ScalableUI 側で横取りする設計は避けるべきである。
...
9. 対象環境 AAOS baseline との差分をレビューする。
10. 実機で focus、input、UX restriction、multi-user、multi-display を評価する。

このフローを個人検証側 CI に乗せられると、対象環境へ渡す前に HMI 仕様の破綻をかなり検出できる。
...
| 優先度 | 内製対象 | 理由 |
| --- | --- | --- |
| 1 | Panel catalog | HMI 意図そのものなので個人検証側が持つべき |
| 2 | RRO / Panel XML | 画面構成を直接表現でき、差分も追いやすい |
| 3 | default activity assignment | 個人検証側アプリとの接続点になる |
| 4 | static transition / state | 仕様と実装のズレを減らせる |
| 5 | smoke test | 対象環境受け入れ条件にできる |
| 6 | event bridge | dynamic 制御が必要になった段階で追加 |
| 7 | controller / task routing | 対象環境と共同設計してから着手 |

### 対象環境主担当のまま残すべき範囲

内製化しても、次の領域は対象環境主担当として残すのが自然である。

| 領域 | 理由 |
| --- | --- |
| 実車用 product makefile 反映 | 車両構成、SKU、partition 設計に依存する |
| platform 署名 | 対象環境 / project の署名管理領域 |
| privapp-permissions | 本格適用セキュリティポリシーに依存する |
...
| --- | --- |
| HMI の試行錯誤が多い | 内製すべき |
| 個人検証側アプリと Panel assignment の結合が強い | 内製すべき |
| 対象環境の変更リードタイムが長い | 内製すべき |
| 対象 branch が頻繁に変わる | patch 管理体制が必要 |
| controller まで頻繁に変わる | 共同開発体制が必要 |
| 車両依存が非常に強い | 対象環境主導を残す |

実務的には、まず個人検証側で RRO/XML と emulator 検証を持ち、controller/task routing は PoC で必要な範囲だけ参照実装する。そのうえで、対象環境へ「この patch set を本格適用 baseline に取り込んでください」と渡せる状態を作るのが最も進めやすい。

## できること

### アプリ内 HMI の設計

個人検証側アプリの Activity 内に表示される UI は個人検証側で設計できる。例えば地図、メディア、カレンダー、空調、車両設定などのアプリについて、ScalableUI の Panel に収まることを前提にレスポンシブなレイアウトを作ることは可能である。
...
個人検証側アプリは、全画面専用ではなく、複数サイズの Panel に配置される前提で作ることができる。
...
この設計は個人検証側で主導できる。ただし、Panel のサイズ、位置、最大化アニメーション、他 Panel の退避制御は SystemUI / ScalableUI 側の責務である。

### ScalableUI 向け HMI 仕様の作成

個人検証側で ScalableUI の仕様入力として以下を定義できる。
...
これはアプリレイヤーだけで完結する実装ではないが、個人検証側が HMI の意図を具体化して対象環境へ渡すうえで重要である。

### アプリ選択 UI のプロトタイプ

Home の customizable panel に表示するアプリをユーザーが選択する仕様は、個人検証側でプロトタイプ可能である。

ただし、実際に選択したアプリを別 Panel の Task として表示するには、以下のどちらかが必要である。

| 方式 | 内容 | 本格適用 |
| --- | --- | --- |
| 対象環境提供の ScalableUI bridge を呼ぶ | 個人検証側アプリが選択結果を Intent / API で SystemUI に渡す | 推奨 |
| 個人検証側アプリ内で疑似的に表示する | 同一 Activity 内でカードや Fragment として見せる | PoC 向け |
...
Panel、TaskPanel、DecorPanel の定義は SystemUI / RRO / ScalableUI 設定側に属する。通常 APK として提供される個人検証側アプリから、本格適用 AAOS の Panel XML を直接変更することはできない。
...
```mermaid
flowchart LR
    P[Panel] --> TP[TaskPanel / TaskView 相当]
    TP --> T[Task]
    T --> ACT[Activity]
```
...
個人検証側アプリは復帰イベントを要求する Intent を送れる可能性はあるが、実際の Panel 復帰は SystemUI / ScalableUI 側の実装である。
...
| WMShell 連携 | TaskView、TaskOrganizer、WindowContainerTransaction 連携が必要 |
...
| SELinux / privapp-permissions | 実機向けイメージでの権限付与が必要 |

## 個人検証側で織り込めるデザイン・カスタマイズ例

### 固定マルチパネル Home

地図、メディア、カレンダー、車両状態などを初期 Panel に配置する Home 画面。個人検証側は各アプリの画面と Panel 別表示モードを定義し、対象環境側が ScalableUI の Panel 定義と default activity assignment に反映する。

### All Apps フローティング Panel

All Apps を画面中央の overlay Panel として表示し、外側タップまたはグリッドアイコン再タップで閉じる仕様。個人検証側は All Apps UI と起動 Intent 仕様を定義し、対象環境側が overlay Panel、scrim、z-order、dismiss event を実装する。

### アプリ最大化と Home 復帰

既に Panel に表示されているアプリを再度選択した場合、その Panel を最大化し、他 Panel を押しのけるように退避させる仕様。個人検証側は最大化時のアプリ UI を準備し、対象環境側が ScalableUI transition と Home 復帰状態を実装する。

### customizable panel

ユーザーが表示アプリを選択できる Panel。個人検証側は選択 UI、候補アプリ、ユーザー設定のデータモデルを設計できる。選択結果を実際の TaskPanel に反映する処理は、対象環境側の ScalableUI bridge または SystemUI 拡張が必要である。

### モード別 HMI

Drive、Park、Charge、ADAS、Reverse などの状態に応じて Panel の表示・優先度・操作可否を変更する仕様。個人検証側は状態ごとの HMI 要求を定義し、対象環境側が車両状態、UX Restrictions、ScalableUI state transition と接続する。

## 推奨する進め方

### 1. 個人検証側で HMI 仕様を ScalableUI 前提に分解する
...
### 2. 対象環境に渡す integration contract を作る

個人検証側アプリが SystemUI に依頼するイベントを明確にする。
...
| Panel 構成案 | 主担当 | レビュー |
| ScalableUI XML / RRO | 主担当化を推奨 | 実機環境への取り込み |
...
個人検証側が ScalableUI を「直接使って HMI を完成させる」というより、ScalableUI を前提にした HMI 仕様、アプリ UI、イベント契約、アプリ適応方針を作り、対象環境が AAOS システム側へ統合する形が現実的である。

個人検証側単独で完結できるのは、Panel に載る各アプリの体験品質と、ScalableUI へ渡すべき HMI 要求の具体化である。ScalableUI の Panel 生成、Task 配置、transition、layer、focus、Home 復帰などは、本格適用では対象環境統合領域として扱うべきである。
