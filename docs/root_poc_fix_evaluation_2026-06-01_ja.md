...
注記: AOSP の `WindowContainerTransaction.reparent(...)` は存在しますが、現在の live ScalableUI source だけでは Panel 間既存 task reparent を標準機能として確認していません。reparent は PoC custom / OEM policy として再検証してください。
