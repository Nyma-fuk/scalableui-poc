...
event 名や token schema は PoC / OEM custom になる場合があります。量産では送信元制限、permission、spoofing 対策も必要です。
...
ただし、任意 panel の追加 UI、geometry、永続化、app picker、drag preview は ScalableUI 標準だけでは完結しません。PoC/OEM custom として扱います。
